...
const PropertyDetail = () => {
    ...
    useEffect(() => {
        const fetchProperty = async () => {
            ...
            try {
                ...
                const priceValue = parseNumber(data.data.price) ?? parseNumber(data.data.priceSale) ?? parseNumber(data.data.priceRent) ?? data.data.priceNumber ?? 0;
    ...
    const title = property.name || `${property.type || 'อสังหา'} ใน ${property.address || 'ไม่ระบุที่อยู่'}`;
    ...
    const saleTypeLabel = property.saleType ? property.saleType : 'ขาย/เช่า';
    const bedroomValue = property.bedrooms ?? (property.rooms ? parseNumber(property.rooms) : '-')
    ...
    const typeLabel = property.type || property.property_type || '-'
    const dateText = property.date || property.post_at || '';
                ...
                <div>
                    ...
                    <Col md={5} className='pr-4'>
                        ...
                        <InputGroup className='mb-3' style={{ fontWeight: 'bold', fontSize: 28 }}>
                            ...
                            <Form.Control value={priceInput} onChange={(e) => setPriceInput(e.target.value)} type='number' />
                        </InputGroup>
                        <p style={{ marginTop: '2rem', fontWeight: 'bold', fontSize: 14 }}>อัตราดอกเบี้ย</p>
                        <InputGroup className='mb-3' style={{ fontWeight: 'bold', fontSize: 28 }}>
                            <InputGroup.Text>%</InputGroup.Text>
                        ...
                        <Row className='mt-4'>
                            <Col xs={6}>
                                <p style={{ fontWeight: 'bold', fontSize: 14 }}>ระยะเวลากู้ (ปี)</p>
                                <InputGroup className='mb-3' style={{ fontWeight: 'bold', fontSize: 28 }}>
                                    <Form.Control value={loanTerm} onChange={(e) => setLoanTerm(Number(e.target.value))} type='number' />
                                </InputGroup>
                            </Col>
                            <Col xs={6}>
                                <p style={{ fontWeight: 'bold', fontSize: 14 }}>เงินดาวน์ (%)</p>
                                <InputGroup className='mb-3' style={{ fontWeight: 'bold', fontSize: 28 }}>
                                    <InputGroup.Text>%</InputGroup.Text>
                        ...
                        <div className='d-grid mt-4'>
                            <Button onClick={() => setLoanResult(calculateLoan(parseNumber(priceInput) ?? 0, downPercent, interestRate, loanTerm))} className='rounded-pill' style={{ backgroundColor: GOLD_COLOR, color: '#000', fontWeight: 'bold', fontSize: '1.5rem', padding: '0.8rem 3rem', border: 'none' }}>คำนวณอีกครั้ง</Button>
                        </div>
                    </Col>
                    <Col md={7} className='pl-5 pt-4'>
                        ...
                        <div className='d-flex justify-content-between mb-5'>
                            <div className='d-flex align-items-center'>
                                ...
                                <span style={{ fontWeight: 'bold', fontSize: 16 }}>฿ {formatCurrency(loanResult.downpayment)} เงินดาวน์</span>
                            </div>
                            <div className='d-flex align-items-center'>
                                <span style={{ height: '12px', width: '12px', backgroundColor: BLUE_COLOR, borderRadius: '50%', marginRight: '8px' }}></span>
                                <span style={{ fontWeight: 'bold', fontSize: 16 }}>฿ {formatCurrency(loanResult.loanAmount)} สินเชื่อ</span>
                            </div>
                        </div>
                        <h3 style={{ fontWeight: 'bold', fontSize: 20, marginBottom: '0.5rem' }}>ยอดผ่อนชำระต่อเดือน</h3>
                        <p className='text-muted' style={{ fontSize: 14 }}>คำนวณจากอัตราดอกเบี้ยและระยะเวลากู้</p>
